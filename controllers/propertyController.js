...
export const allProperty = async (req, res, next) => {
  try {
    const { categoryId, subCategoryId, startDate, endDate, maxGuests } =
      req.body;
    ...
    if (maxGuests) {
      filters.maxGuests = { $lte: maxGuests };
    }
...
export const singleProperty = async (req, res, next) => {
  try {
    ...
    res
      ...
      .json({
        success: true,
        message: "Propertie Fetched successfully",
        data: property,
      });
  } catch (error) {
    next(error);
...
export const deleteProperty = async (req, res, next) => {
  try {
    const property = await Property.findOneAndUpdate(
      { _id: req.params.propertyId, isDeleted: false },
      { isDeleted: true },
      { new: true }
    );
    if (!property) {
